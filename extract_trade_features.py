...
def extract_features(csv_file):
    ...
    def find_col(keywords, columns):
        ...
        if not matches:
            raise ValueError(f"未找到包含关键字 {keywords} 的列，实际列名: {list(columns)}")
        return matches[0]

    time_col = find_col(["时间", "time", "timestamp", "open_time", "close_time"], columns)
    # 可能用不上，但保留
    direction_col = find_col(["方向", "dir", "cmd", "side"], columns) if any(
        k in c for c in columns for k in ["方向", "dir", "cmd", "side"]
    ) else None
    ...
    volume_changes_mean = float(volume_changes_series.mean()) if total_trades > 0 else 0.0
    volume_changes_std = float(volume_changes_series.std(ddof=1)) if total_trades > 1 else 0.0
    ...
    try:
        if total_trades >= 2:
            ...
            double_volume_flags = np.isclose(next_volumes, 2.0 * prev_volumes, rtol=1e-6, atol=1e-12)
            double_volume_count = int(np.sum(double_volume_flags))
            double_volume = double_volume_count > 0 and double_volume_count >= (total_trades - 1) / 2.0
        else:
            double_volume = False
    except Exception:
        double_volume = False
    ...
    holding_periods_mean = float(holding_periods_series.mean()) if total_trades > 0 else 0.0
    holding_periods_std  = float(holding_periods_series.std(ddof=1)) if total_trades > 1 else 0.0
    holding_periods_max  = float(holding_periods_series.max()) if total_trades > 0 else 0.0
    holding_periods_min  = float(holding_periods_series.min()) if total_trades > 0 else 0.0
    ...
    profit_factor = float(sum_wins / sum_losses_abs) if sum_losses_abs > 0 else (np.inf if sum_wins > 0 else 0.0)

    payoff_ratio = float(avg_win / abs(avg_loss)) if avg_loss < 0 else (np.inf if avg_win > 0 else 0.0)

    expectancy_per_trade = float(win_rate * avg_win + (1 - win_rate) * (avg_loss if avg_loss != 0 else 0.0))
    ...
    def rolling_edge_binary(s01: pd.Series, window: int):
        # prior (t-1) 的胜率滚动均值
        return s01.shift(1).rolling(window=window, min_periods=max(5, window // 3)).mean()
    ...
    denom = equity_abs.clip(lower=np.nanpercentile(equity_abs.dropna(), 5) if equity_abs.notna().any() else 1.0)
    ...
    corr_spearman = {}
    for w in (10, 20, 50, 100):
        corr_linear[f"w{w}"] = safe_corr(df[volume_col], rolling_edges[w], method="pearson")
        corr_spearman[f"w{w}"] = safe_corr(df[volume_col], rolling_edges[w], method="spearman")

    corr_volume_with_prior_equity = safe_corr(df[volume_col], prior_equity, method="pearson")
    corr_volume_with_prior_equity_spear = safe_corr(df[volume_col], prior_equity, method="spearman")

    # 同样计算 “仓位占权益” 与边/权益的相关（更接近 Kelly 的下注比例逻辑）
    corr_frac_linear = {}
    corr_frac_spearman = {}
    for w in (10, 20, 50, 100):
        corr_frac_linear[f"w{w}"] = safe_corr(vol_frac_of_equity, rolling_edges[w], method="pearson")
        corr_frac_spearman[f"w{w}"] = safe_corr(vol_frac_of_equity, rolling_edges[w], method="spearman")

    corr_frac_with_prior_equity = safe_corr(vol_frac_of_equity, prior_equity, method="pearson")
    corr_frac_with_prior_equity_spear = safe_corr(vol_frac_of_equity, prior_equity, method="spearman")
    ...
    vol_change_after_loss_mean = float(vol_change[prev_is_loss].mean()) if prev_is_loss.any() else np.nan
    vol_change_after_win_mean  = float(vol_change[prev_is_win].mean())  if prev_is_win.any() else np.nan
    prob_vol_increase_after_loss = float((vol_change[prev_is_loss] > 0).mean()) if prev_is_loss.any() else np.nan
    prob_vol_increase_after_win  = float((vol_change[prev_is_win] > 0).mean())  if prev_is_win.any()  else np.nan
    ...
    avg_vol_after_win  = float(df[volume_col][prev_is_win].mean())  if prev_is_win.any()  else np.nan
    avg_vol_after_loss = float(df[volume_col][prev_is_loss].mean()) if prev_is_loss.any() else np.nan
    vol_ratio_win_vs_loss = mean_ratio(df[volume_col][prev_is_win], df[volume_col][prev_is_loss])
    ...
    volume_change_iqr        = float((vol_change.quantile(0.75) - vol_change.quantile(0.25))) if total_trades > 0 else np.nan
    ...
    # 滚动波动性（小 → 更平滑）
    vol_roll_std_w10  = float(df[volume_col].rolling(10, min_periods=5).std().mean()) if total_trades > 0 else np.nan
    vol_roll_std_w20  = float(df[volume_col].rolling(20, min_periods=7).std().mean()) if total_trades > 0 else np.nan
    ...
    def rolling_kelly_fraction(outcomes01: pd.Series, pnl: pd.Series, w: int):
        ...
        s = pd.DataFrame({"o": outcomes01.astype(float), "p": pnl.astype(float)}).shift(1)
        ...
        avg_loss_roll = losses.rolling(window=w, min_periods=max(10, w // 2)).mean()  # 负
    ...
    kelly_beta = {}
    for w in (20, 50, 100):
        kelly_alignment[f"w{w}_pearson"]  = safe_corr(vol_frac_of_equity, fpos_w[w], method="pearson")
        kelly_alignment[f"w{w}_spearman"] = safe_corr(vol_frac_of_equity, fpos_w[w], method="spearman")
        kelly_beta[f"w{w}"] = safe_beta(fpos_w[w], vol_frac_of_equity)

    # ---- 回撤敏感度（回撤期是否缩仓：Kelly 常见行为）----
    in_drawdown = (drawdown > 0)
    avg_frac_in_dd  = float(vol_frac_of_equity[in_drawdown].mean()) if in_drawdown.any() else np.nan
    avg_frac_out_dd = float(vol_frac_of_equity[~in_drawdown].mean()) if (~in_drawdown).any() else np.nan
    frac_ratio_out_vs_in_dd = (avg_frac_out_dd / avg_frac_in_dd) if (avg_frac_in_dd not in [0, None, np.nan]) else np.nan
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("用法: python extract_trade_features.py <csv_file_path> [output_json_path]")
        sys.exit(1)
    csv_file = sys.argv[1]
